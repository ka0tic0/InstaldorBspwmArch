...
install_yay() {
    ...
    log_info "Compilando yay desde AUR..."
...
install_packages() {
    ...
    if command -v yay &> /dev/null; then
        yay -S --needed --noconfirm \
            ...
            ttf-nerd-fonts-symbols-2048-em-mono || log_warning "Algunos paquetes de AUR no están disponibles"
    else
        log_warning "yay no disponible, omitiendo paquetes de AUR"
    fi
...
EOF
    ...
    log_success "GTK configurado (3.0 y 4.0)"
}
    ...
    if [[ -n "$selections" ]]; then
        for selection in $selections; do
            if [[ -n "${package_map[$selection]}" ]]; then
                log_info "Instalando ${package_map[$selection]}..."
                pacman -S --noconfirm "${package_map[$selection]}" || log_warning "No se pudo instalar ${package_map[$selection]}"
            fi
        done
    fi
}
